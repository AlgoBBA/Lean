    ...
    public class LeanOptimizerTests
    {
        ...
        public void TrackEstimation()
        {
            ...
            optimizer.Update += (s, e) =>
            {
                ...
                Assert.LessOrEqual(packet.MaximumConcurrentBacktests, int.Parse(runtimeStats["Running"], CultureInfo.InvariantCulture));
                ...
                if (completedTests > 0)
                {
                    Assert.Greater(TimeSpan.Parse(runtimeStats["Average Length"], CultureInfo.InvariantCulture), TimeSpan.Zero);
                }
